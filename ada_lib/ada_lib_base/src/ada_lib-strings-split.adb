...
-----------------------------------------------------------------------------
--
--  Note: Originally unbounded strings were used here, but I changed
--  that to Varying_Strings because I've found Strings_Unbounded to
--  be *really* slow when appending a character at a time.  Plus
--  we shouldn't need to split up large strings, and the maximum
--  defined below should give a decent upper bound.

with Ada_Lib.Strings.Varying; use Ada_Lib.Strings.Varying;

package body Ada_Lib.Strings.Split is

   Maximum_Result_Length        : constant := 2048;
   ...
   function Select_Fields (Line   : String;
                           Format : String) return String
   is
      Result : Varying_String_Type (Maximum_Result_Length);
   ...
   function Build_String (Line   : String;
                          Format : String) return String
   is
