                 ...
                 case Status is
                     ...
                     when GNAT.Sockets.Expired =>
                        ...
                        if Timeout > 0.0 and then Database.Socket_Opened then
                           ...
                           raise Timed_Out with
                              "timeout waiting for socket read. timrout " &
                              Timeout'img;
                        else
                           exit;
                        end if;
...
-- end Initialize;
   ...
   begin
    ...
    ---------------------------------------------------------------------------
      ...
      Result   : constant Boolean := Database.Socket_Opened and then
                                       Database.Selector_Created;
    begin
      return Log_Here (Result, Trace_All,
