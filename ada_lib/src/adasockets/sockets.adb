...
package body Sockets is
   ...
   begin
	  ...
	  loop
      Count := C_Recvfrom (Socket.FD, Buffer'Address, Buffer'Length, MSG_NOSIGNAL,
   ...
   begin
      while Rest > 0 loop
         Count := C_Recvfrom (Socket.FD, Data (Index) 'Address,
                              int (Rest), MSG_NOSIGNAL, Addr'Address, Addrlen'Access);
         if Count < 0 then
		 	if Thin.Errno /= EINTR then
            	Raise_With_Message ("Receive error");
			end if;
   ...
   begin
      ...
      while Rest > 0 loop
         Count := C_Send (Socket.FD, Data (Index) 'Address, int (Rest), MSG_NOSIGNAL);
